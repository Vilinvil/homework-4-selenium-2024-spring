...
from ui.pages.base_page_functionality import BasePageFunctionality, add_hover, add_write
...
class OverviewNewUserPage(BasePageFunctionality):
    ...
    def START_ACTION_BY_TITLE(self, title):
        return self.find_with_check_visibility(self.locators.START_ACTION_BY_TITLE(title))

    def check_display_start_actions(self):
        assert self.START_ACTION_BY_TITLE('Создать кампанию вручную')
        assert self.START_ACTION_BY_TITLE('Пройти обучение')
...
class OverviewPage(PageWithView, PageWithRedirectWindow):
    ...
    def check_display(self):
        ...
        assert self.BUTTON_CHOOSE_CAMPAIGNS
